...
public class FPController : MonoBehaviour
{
    ...
    {
       if (GameStats.gameOver == true) return;
       ...
       if (health <= 0)
       ...
       else
        {
            ...
            float bloodScale = Random.Range(0.5f, 1.5f);
            bloodSpatter.transform.localScale = new Vector3(bloodScale, bloodScale, 1);
            bloodSpatter.transform.position = new Vector3(Random.Range(0, cWidth), Random.Range(0, cHeight), 0);
            Destroy(bloodSpatter, 2.2f);
        }
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.tag == "Home")
        {
            ...
            GameStats.gameOver = true;
            Destroy(this.gameObject);
            GameObject gameOver_Txt = Instantiate(gameOverPrefab);
            gameOver_Txt.transform.SetParent(canvas.transform);
            gameOver_Txt.transform.localPosition = Vector3.zero;
        }
    ...
    void ProcessZombieHit()
    {
        ...
        if(Physics.Raycast(shotDirection.position, shotDirection.forward, out hitInfo, 200, ~checkPointLayer))
        {
            ...
            //GameObject zombieCtrl = hitZombie.GetComponent<ZombieController>().gameObject;
            if (hitZombie.tag == "Zombie")
            {
                ...
                { 
                    if (Random.Range(0, 10) < 5)
                    {
                        ...
                        newRd.transform.Find("Hips").GetComponent<Rigidbody>().AddForce(Camera.main.transform.forward * 10000);
                        Destroy(hitZombie);
                    }
                    else
                    {
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && !anim.GetBool("fire") && anim.GetBool("arm") && GameStats.canShoot == true)
        {
            if (ammoClip > 0)
            {
